...
if (filtro !== 'top6' && j.categoria !== filtro) return;const card = document.createElement("div");card.className = game-card ${j.top6 ? 'top6' : ''};card.innerHTML = <div> <div class="game-meta"><span>⚽ ${j.liga}</span><span>${j.data}</span></div> <div class="game-title">${j.equipas}</div> <div class="suggestion-box"> <small style="color:#666;">🎯 Sugestão:</small> <div style="font-weight:bold; color:var(--dark); margin:2px 0;">${j.dica}</div> <span style="background:var(--primary); color:white; padding:2px 6px; font-size:11px; border-radius:4px; font-weight:bold;">Odd: ${j.odd.toFixed(2)}</span> </div> </div> <button class="btn-import" onclick="importarParaFormulario('${j.equipas} (${j.dica})', ${j.odd})">⚡ Importar Odd</button>;container.appendChild(card);});}function importarParaFormulario(evento, odd) {mudarSeparador('gestor');const tipo = document.getElementById("form-tipo").value;if (tipo === 'simples') {document.querySelector(".input-evento").value = evento;document.querySelector(".input-odd").value = odd;} else {const rows = document.querySelectorAll(".multi-game-row");let preenchido = false;for (let i = 0; i < rows.length; i++) {const evInp = rows[i].querySelector(".input-evento");const oddInp = rows[i].querySelector(".input-odd");if (!evInp.value) {evInp.value = evento;oddInp.value = odd;preenchido = true;break;}}if (!preenchido) {adicionarLinhaJogoForm();const novosRows = document.querySelectorAll(".multi-game-row");const ultimaLinha = novosRows[novosRows.length - 1];ultimaLinha.querySelector(".input-evento").value = evento;ultimaLinha.querySelector(".input-odd").value = odd;}}calcularOddTotalMultipla();document.getElementById("form-valor").focus();}function sair() {document.getElementById("password").value = "";document.getElementById("login-box").style.display = "block";document.getElementById("private-dashboard").style.display = "none";document.body.style.alignItems = "center";}
