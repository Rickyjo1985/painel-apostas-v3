...
async function apiFetch(path, params = {}) {
  ...
  if (!key) {
    throw new Error("APIFOOTBALL_KEY não está disponível neste deployment. Confirma a Environment Variable e faz um novo Redeploy.");
  }
  ...
  catch { data = { errors: { api: text || `HTTP ${res.status}` } }; }

  if (!res.ok || (data.errors && Object.keys(data.errors).length)) {
    const errors = data.errors ? JSON.stringify(data.errors) : `HTTP ${res.status}`;
    if (res.status === 401 || res.status === 403 || /missing application key|invalid.*key|unauthorized/i.test(errors)) {
      throw new Error("A API-Football recusou a chave. Confirma que APIFOOTBALL_KEY contém a chave da API-Football correcta e faz novo Redeploy.");
    }
...
export default async function handler(req, res) {
  try {
    ...
    const force = String(rawQuery.force || "") === "1";
